...
corotTruss2D::update(void)
{
	...
	//Ln = (currentCrd2 - currentCrd1).Norm();
	//Compute engineering strain and strain rate
	double strain = (Ln - Lo) / Lo;
	/*
	//OR caculate strain like this:
	double dLength = 0.0;
	for (int i = 0; i < 2; i++) {
		double deltaDisp = end2Disp(i) - end1Disp(i);
		dLength += deltaDisp *cosX[i];
	}
	double strain = dLength / Lo;
	*/
	...
	/*
	double A = crdNode2(1) - crdNode1(1);
	double B = crdNode1(0) - crdNode2(0);
	double C = crdNode1(1)*crdNode2(0) - crdNode2(1)*crdNode1(0);
	double u2 = (A* currentCrd1(0) + B*currentCrd1(1) + C) / (sqrt(pow(A, 2) + pow(B, 2)));
	double u5 = (A* currentCrd2(0) + B * currentCrd2(1) + C) / (sqrt(pow(A, 2) + pow(B, 2)));

	double u2X = (pow(B, 2)*currentCrd1(0) - A * C - A *B* currentCrd1(1)) / (pow(A, 2) + pow(B, 2));
	double u2Y = (pow(A, 2)*currentCrd1(1) - B * C - A * B* currentCrd1(0)) / (pow(A, 2) + pow(B, 2));

	double u5X = (pow(B, 2)*currentCrd2(0) - A * C - A * B* currentCrd2(1)) / (pow(A, 2) + pow(B, 2));
	double u5Y = (pow(A, 2)*currentCrd2(1) - B * C - A * B* currentCrd2(0)) / (pow(A, 2) + pow(B, 2));
	
	double deltaUx1 = sqrt(pow(u5X - currentCrd2(0)	, 2) + pow(u5Y - currentCrd2(1), 2))- sqrt(pow(u2X - currentCrd1(0), 2) + pow(u2Y - currentCrd1(1), 2));
	...
	*/
	...
	double a = acos(cosX[0]) * 180 / 3.14159265358979323846;  //a<0,pi>
	if (crdNode1(1) > crdNode2(1)) {
		a = 360 - a;
	}

	double cosb = (currentCrd2(0) - currentCrd1(0)) / Ln;
	double b= acos(cosb) * 180 / 3.14159265358979323846;

	if (currentCrd1(1) > currentCrd2(1)) {
		b = 360 - b;
	}


	double beta = (b - a)*3.14159265358979323846 / 180;
